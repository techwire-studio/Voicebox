...
const BookNow = () => {
	...
	useEffect(() => {
		if (
			...
			gsap.from(formButtonRef.current, {
				...
				ease: "back.out(1.7)",
				delay: isMobile
					? formFieldsRef.current.length * 0.15 + 0.2
					: formFieldsRef.current.length * 0.1 + 0.2,
				scrollTrigger: {
					trigger: formSectionRef.current,
					start: "top 75%",
					toggleActions: "play none none none",
	...
	return (
		<div className="w-full">
			...
			<div
				ref={formSectionRef}
				className="py-8 px-4 lg:px-0 lg:py-16 lg:w-[60%] m-auto">
			...
			<div
				...
				className="w-full bg-choose_bg px-4 py-4 lg:px-8 lg:py-24">
					...
					<div
						...
						{links.map((link, index) => (
							<div
								...
								<img
									className="w-16 bg-white p-4 rounded-full"
									src={link.image}
									alt="link-img"
									loading="lazy"
								/>
								<p className="mt-4 text-white font-jost text-body-mobile lg:text-body-desktop">
									{link.title}
								</p>
								<p className="mt-4 text-service_bg font-jost text-subtext-mobile lg:text-subtext-desktop">
									{link.content}
								</p>
							</div>
						))}
